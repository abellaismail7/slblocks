...
int battery_remaining(const char *bat,uintmax_t* h,uintmax_t* m);
...
const char * battery(void){
	...
	perc = (perc / 74) * 100; // to have more battery life manufacter charges only 75% 
	sprintf(buf,"%s %d%%", icon, perc);
	return buf;
}
	...
	int battery_remaining(const char *bat,uintmax_t* h,uintmax_t* m)
	{
		uintmax_t charge_now, current_now;
		double timeleft;
		char path[PATH_MAX];
